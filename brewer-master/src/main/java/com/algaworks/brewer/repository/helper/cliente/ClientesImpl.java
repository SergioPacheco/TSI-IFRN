...
public class ClientesImpl implements ClientesQueries {
	...
	public Cliente buscarComCidadeEstado(Long codigo) {
		Criteria criteria = manager.unwrap(Session.class).createCriteria(Cliente.class);		
		criteria.createAlias("endereco.cidade", "c", JoinType.LEFT_OUTER_JOIN);
		// errado: criteria.createAlias("endereco.cidade.estado", "e", JoinType.LEFT_OUTER_JOIN);
		criteria.createAlias("c.estado", "e", JoinType.LEFT_OUTER_JOIN); //certo
		criteria.add(Restrictions.eq("codigo", codigo));
		//criteria.setResultTransformer(Criteria.DISTINCT_ROOT_ENTITY);//Saber o que essa linha faz só por curiosidade porque um aluno tentou fazer assim
