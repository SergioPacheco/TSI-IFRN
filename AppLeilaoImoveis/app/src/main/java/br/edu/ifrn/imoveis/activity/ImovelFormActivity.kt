...
class ImovelFormActivity : AppCompatActivity() {
    ...
    override fun onCreate(icicle: Bundle?) {
        ...
        setupToolbar(R.id.toolbar, imovel?.descricao?: getString(R.string.novo_imovel))
    ...
    fun initViews() {
        ...
        imovel?.apply {
            // Foto do imovel
            appBarImg.loadUrl(imovel?.foto)
            // Dados do imovel
            tDesc.string = descricao
            // Tipo do imovel
            when(tipo) {
                "casa" -> radioTipo.check(R.id.tipoCasa)
                "apartamento" -> radioTipo.check(R.id.tipoApartamento)
                "terreno" -> radioTipo.check(R.id.tipoTerreno)
            }
        }
    }
    ...
    private fun taskSalvar() {
        ...
        doAsync {
            ...
            c.descricao = tDesc.string
            c.tipo = when (radioTipo.checkedRadioButtonId) {
                R.id.tipoCasa -> TipoImovel.casa.name
                R.id.tipoApartamento -> TipoImovel.apartamento.name
                else -> TipoImovel.terreno.name
            }
